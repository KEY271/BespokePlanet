...
$$
T_a=T_N\left(\frac{p_s}{p_N}\right)^\kappa
$$

を地上気温とよび、陸海タイルを持つケースでは格子点ごとの月平均を `surface_air_temperature` として出力する（[陸海ケースの出力](../cases/land-sea.md#出力)）。$T_a$ は顕熱が地面温度と比べる空気の温度そのもので、全タイルに共通の格子の値である。ビジュアライザのケッペン区分と雨温図は、地面（陸タイルの表面）の温度 $T_L$ ではなくこの $T_a$ を使う。日射で暖まった裸地では $T_L$ が空気より数 K 以上高くなり、観測の地上 2 m 気温で定義された区分とずれるためである。$T_a$ は最下層（地表から約 $600\,\mathrm m$）の空気を乾燥断熱で下ろした値なので、安定成層では実際の地上気温より高めに出る。
