...
$C,\alpha,K$ は時間変化しない。$\beta_L$ は時間変化するが、他のフラックスと同じく[物理過程を評価する時刻](./physics-time-level.md)の $\overline W^{n-1}$ から求める。$T_s,T_d$ は[地面](./ground.md)と同じ LeapFrog で進め、超粘性と重力波の処理は適用せず、RAW フィルターは適用する。$W$ の更新は[バケツモデル](./bucket.md#評価する時刻と数値更新)のとおりとする。
