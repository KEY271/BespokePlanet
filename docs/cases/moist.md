...
- [雲](../tendency/cloud.md)の診断を有効にし、対流調節と大規模凝結の後の相対湿度と対流性降水から、大規模雲と対流雲の面積率と高さを診断する。放射は晴天・大規模雲・対流雲の 3 つの副気柱で解き、雲は長波では黒体、短波では雲頂にある吸収のない δ-Eddington の層（ISCCP の全球平均の光学的厚さ $\tau=3.9$、非対称因子 $g=0.85$）とし、雲と下の大気・地表の間の多重反射を解く（[帯域別放射](../tendency/band-radiation.md#6-雲の種類と副気柱)）。雲による短波の吸収は扱わない。海面のアルベドは雲を繰り込んだ $0.3$ から開いた海面の $\alpha_o=0.06$ に変える。
