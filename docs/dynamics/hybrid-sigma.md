...
$p_0=1000\,\mathrm{hPa}$ を基準とし、上端気圧を $p_{1/2}=1\,\mathrm{hPa}$ とする。$N=12$ では $A,B$ は

$$
\begin{aligned}
A_{k+1/2}&=100,300,1000,5000,10000,8000,8000,10000,12000,10000,7000,3000,0\,\mathrm{Pa}\\
B_{k+1/2}&=0,0,0,0,0,0.1,0.2,0.3,0.4,0.55,0.7,0.85,1
\end{aligned}
$$
...
と対応づけ、$p_s=p_0$ のとき $p_{k+1/2}=\eta_{k+1/2}p_0$ となるようにする。上端は $\eta_{1/2}=p_T/p_0=0.001$ である。予報変数を置く full level では $\eta_k=(\eta_{k-1/2}+\eta_{k+1/2})/2$ とする。
