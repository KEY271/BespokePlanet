...
となる。外力 $\bm{F}_k$ は摩擦などの物理過程から与える。ただし
...
基準大気からのずれを重力波として分離して陰的に解く。重力波演算子の参照温度は全層で $\widetilde T_k=300\,\mathrm{K}$ とし、地表面気圧は $p_s=p_0=10^5\,\mathrm{Pa}$、$\zeta=\delta=0$ とする。この一定値を以下の一般形に代入して用いるため、温度の鉛直差を含む式自体は変更しない。初期温度場も変更せず、重力波の線形演算子に用いる参照温度だけを $300\,\mathrm{K}$ とする。地表ジオポテンシャル $\Phi_s$ は時間変化しないので基準大気からのずれには含めず、$-\nabla_\eta^2\Phi_s$ は後述の元の方程式の右辺 $\mathcal{R}(X)$ にだけ現れる。したがって $\Phi_s$ を与えても下の線形演算子と陰的に解く行列は変わらない。基準大気の量にはチルダをつけて表し、基準大気からのずれはプライムをつけて表すと、線形化した方程式は
...
となる。物理過程を持たない場合は $\mathcal{P}=0$ である。
...
と書く。初期値 $X^0$ と地表ジオポテンシャル $\Phi_s$ は外から与える。$\Phi_s$ は積分中ずっと固定し、超粘性や RAW フィルターも適用しない。最初の二時刻は
