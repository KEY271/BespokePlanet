...
放射ケースでは上層で励起された重力波が発散場に蓄積し、流速が異常に増大することがある。そこで発散 $\delta$ に限り、上層ほどラプラシアンの次数を下げる。次数を下げると切断波数での減衰率を変えずに中間波数の減衰が強くなるため、上層の重力波に対するスポンジとして働く。

切断波数を $n_{\max}$ とし、第 $k$ 層の発散の減衰率を

$$
\kappa_{n,k}^{(\delta)}
=\frac{1}{\tau_\delta}
\left[
\frac{n(n+1)}{n_{\max}(n_{\max}+1)}
\right]^{q_k}
$$
...
$$
\tau_\delta=1\,\mathrm{hour}
$$

とする。鉛直方向に変えるのは指数 $q_k$ だけである。hybrid-sigma 座標の nominal full level

$$
\eta_k=\frac{1}{2}\left(
\frac{A_{k-1/2}}{p_0}+B_{k-1/2}
+\frac{A_{k+1/2}}{p_0}+B_{k+1/2}
\right),\qquad p_0=10^5\,\mathrm{Pa}
$$

を使い、

$$
s_k=\max\left(0,\min\left(1,\frac{\eta_{\mathrm{tap}}-\eta_k}{\eta_{\mathrm{tap}}}\right)\right),
$$

$$
q_k=q_{\mathrm{trop}}+s_k(q_{\mathrm{top}}-q_{\mathrm{trop}})
$$

と置く。最初に用いる値は

$$
\eta_{\mathrm{tap}}=0.2,\qquad
q_{\mathrm{trop}}=4,\qquad
q_{\mathrm{top}}=2
$$
...
$5$ 年間、すなわち $5\times360$ 太陽日シミュレーションする。タイムステップは $\Delta t=900\,\mathrm{s}$ とする。このため 1 日は 96 ステップ、1 ヶ月は 2880 ステップ、1 年は 34560 ステップにそれぞれ厳密に一致する。
...
日平均は各太陽日の左端を含み右端を含まない全タイムステップ（96 サンプル）を等重みで平均し、時刻の列にはその日の開始時刻を書く。
...
月平均は各 30 日区間の左端を含み右端を含まない全タイムステップ（2880 サンプル）を等重みで平均する。`m0001` は 1 年目の 4 月、`m0002` は 5 月に対応し、以後も30日ごとに連番とする。
