...
で与えられるが、正確には互いに逆にはならない。係数配列は後述のスペクトル微分のために $n=T+1$ の行（拡張行）も持ち、格子への合成はこの行まで含めて行う。予報変数ではこの行は常にゼロに保つ。
...
経度微分では Fourier 基底を解析的に微分できるため、各係数に $im$ を掛ける：
...
計算では、各 $m$ について $b_{|m|:T+1}^m$ をゼロで初期化し、各入力係数 $a_n^m$（$n=|m|,\dots,T$）に対して
...
$b_{T+1}^m=-T\epsilon_{T+1,|m|}a_T^m$ は一般にはゼロでないため、緯度微分の係数は $n=T+1$ の拡張行まで持ち、格子への合成もこの行まで含めて行う。
