...
$$
\begin{aligned}
a&=6.371\times10^6\,\mathrm{m},
&\Omega&=7.2921159\times10^{-5}\,\mathrm{s^{-1}},\\
...
とし、速度と温度はこの $\eta_k$ で評価する。$N=12$ の係数では

$$
\begin{aligned}
\eta_{k+1/2}&=0.001,0.003,0.01,0.05,0.10,0.18,0.28,0.40,0.52,0.65,0.77,0.88,1,\\
\eta_k&=0.002,0.0065,0.03,0.075,0.14,0.23,0.34,0.46,0.585,0.71,0.825,0.94
\end{aligned}
$$
...
$A(\varphi),B(\varphi)$ の全球平均はどちらも $0$ なので、$\Phi_s$ の全球平均も $0$ である。値は赤道で約 $1106\,\mathrm{m^2\,s^{-2}}$（高さ約 $113\,\mathrm{m}$）、極で約 $-3093\,\mathrm{m^2\,s^{-2}}$（約 $-315\,\mathrm{m}$）となる。
