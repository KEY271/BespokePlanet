...
雲量 $C(\lambda,\varphi)$ を格子で持ち、[湿潤ケース](../cases/moist.md#出力)の出力に日平均の全球平均 $\braket{C}$、月平均の $C(\lambda,\varphi)$、毎年の瞬時値の $C(\lambda,\varphi)$ を加える。瞬時値の $C$ はその時刻の傾向評価で診断した値で、他の格子の瞬時値と同じ時刻に書く。惑星アルベド $\braket{F^{\mathrm{SW}}_{\mathrm{refl}}}/\braket{F^{\downarrow\mathrm{SW}}_{1/2}}$ は既存の短波入射と反射短波の出力から求める。
...
- 雲の設定型（有効・無効、$\mathrm{RH}_c$、$c_0,c_1,P_0,C^{\mathrm{conv}}_{\max}$、$\alpha_c$）を他の物理過程と同じくデフォルト初期化子に一か所で書く。雲は比湿と対流性降水を必要とするので、湿潤過程が有効なケースでだけ有効にできる。
