...
以下は開水面の式である。湿潤・陸海ケースでは海流による熱輸送の代わりに [Q flux](./q-flux.md) を混合層へ加える。湿潤・陸海ケースでは[海氷](./sea-ice.md)を結合し、開水面率をフラックスに掛けて相変化を処理する。海水の熱容量は氷下も含む海全体の値を維持する。乾燥の `slab-ocean` ケースでは海氷は既定で無効である。
...
に従う。海面が大気へ与える長波放射と顕熱には、海洋から差し引くものと同じフラックスを使う。したがって大気と slab ocean を合わせたエネルギー収支は閉じる。海底との熱交換は加えない。この式は乾燥の `slab-ocean` ケースのもので、[Q flux](./q-flux.md) を有効にしたケースでは右辺に時間に依らない $Q(\lambda,\varphi)$ を加える。
