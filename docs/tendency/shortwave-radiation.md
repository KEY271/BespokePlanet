...
が成り立つ。惑星アルベドは $F^{\mathrm{SW}}_{\mathrm{refl}}/F^{\downarrow\mathrm{SW}}_{1/2}$ で、オゾン吸収を無視すれば $C\alpha_c+(1-C\alpha_c)\alpha_s$ である。海上（$\alpha_s=0.06$）で $C=1$ なら $0.464$、$C=0.65$ なら $0.324$、$C=0$ なら $0.06$ になる。
...
となり、雲量に依存しない。数値実装では、Gaussian 求積による大気上端入射量の全球平均が離散化誤差の範囲で $S_0/4$ に一致すること、および上のカラム短波エネルギー収支をテストで確認する。ただし各格子点に与えるフラックスそのものはこの全球平均値ではなく、上記の瞬時値である。
...
- 雲の反射は $F^{\downarrow\mathrm{SW}}_{N+1/2}$ に係数 $(1-C\alpha_c)$ を掛けるだけで、オゾン吸収の計算と大気の加熱は雲がないときと変わらない。$C=0$ のとき、地表に達する短波、地表の吸収、反射短波は雲を入れる前の式にビット単位で一致する。したがって雲を診断しない放射ケースと slab ocean ケースの結果は変わらない。
- 雲と地表で 2 段に反射するが、上向きの短波は扱わないので、雲の底と地表の間の多重反射は含まない。地表アルベドが小さいのでこの寄与は $C\alpha_c\alpha_s(1-C\alpha_c)\lesssim1.5\,\%$ である。

## 実装後の確認項目

- 各気柱で $F^{\downarrow\mathrm{SW}}_{1/2}=\sum_kA^{\mathrm{UV}}_k+F^{\mathrm{SW}}_{\mathrm{abs}}+F^{\mathrm{SW}}_{\mathrm{refl}}$ が丸め誤差の範囲で閉じること。雲のある気柱でも成り立つこと。
- $C=0$ の気柱で、地表に達する短波・地表の吸収・反射短波が雲を入れる前の値とビット単位で一致すること。
- $C=1$ で $\alpha_s=0.06$ の気柱で、反射短波が $F^{\downarrow\mathrm{SW}}_{N+1/2}$ の $\alpha_c+(1-\alpha_c)\alpha_s=0.4642$ 倍になること。
- 同じ入射に対して $C$ を増やすと、反射短波が単調に増え、地表の吸収が単調に減り、大気の UV 加熱は変わらないこと。
- 夜側の格子点で全フラックスと加熱が 0 であること。
