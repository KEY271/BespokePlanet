...
module large_scale_condensation
  ...
  implicit none
  private
  public :: large_scale_condensation_tendency, saturation_condensate
...
contains
  ...
  pure real(real64) function saturation_condensate(config, temperature, humidity, pressure) result(condensate)
    ...
    real(real64) :: saturation, heating_per_condensate, warmed_temperature, residual
    integer :: iteration
    ...
    do iteration = 1, config%maximum_iterations
      ...
      condensate = condensate + residual/(1.0_real64 + heating_per_condensate* &
        saturation_specific_humidity_derivative(warmed_temperature, pressure))
      warmed_temperature = temperature + heating_per_condensate*condensate
      residual = humidity - condensate - saturation_specific_humidity(warmed_temperature, pressure)
  ...
  pure subroutine large_scale_condensation_tendency(config, pressure_half, temperature, humidity, interval, &
    ...
    real(real64) :: full_level_pressure(size(temperature)), delta_pressure(size(temperature)), condensate
    integer :: k
