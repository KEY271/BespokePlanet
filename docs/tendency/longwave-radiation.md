...
状態: **灰色スキーム**。放射を用いるケースは[帯域別放射](./band-radiation.md)を使う。このページの灰色スキームは、設定 `radiation_config%scheme = radiation_scheme_gray` のときの放射で、比較と回帰テストのために残している。[基準水蒸気分布](#基準水蒸気分布)と、地表を黒体とする境界条件は帯域別放射でも共通である。
